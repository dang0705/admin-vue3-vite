  ...
  <el-form
    ...
    <el-form-item class="login-animation3">
      <el-button
        ...
        <span class="text-[white]">登 录</span>
      </el-button>
    </el-form-item>
  </el-form>
</template>
...
const loginRules = reactive({
  mobile: [{ required: true, trigger: 'blur', validator: rule.mobile }],
  code: [
    {
      required: true,
      trigger: 'blur',
...
const handleSendCode = async () => {
  ...
  const response = await sendMobileCode({ phone: loginForm.mobile, type: '1' })
  if (response.data) {
