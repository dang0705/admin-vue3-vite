...
export const login = (data: any) => {
  ...
  // 密码加密
  if (env.VITE_PWD_ENC_KEY) {
    encPassword = other.encryption(data.password, env.VITE_PWD_ENC_KEY)
  }
...
export const loginByMobile = (mobile: any, code: any) => {
  ...
  const scope = 'server'
  const basicAuth = 'Basic ' + window.btoa(env.VITE_OAUTH2_MOBILE_CLIENT)
  Session.set('basicAuth', basicAuth)
...
export const loginBySocial = (state: string, code: string) => {
  const grant_type = 'mobile'
  const scope = 'server'
  const basicAuth = 'Basic ' + window.btoa(env.VITE_OAUTH2_SOCIAL_CLIENT)
  Session.set('basicAuth', basicAuth)
