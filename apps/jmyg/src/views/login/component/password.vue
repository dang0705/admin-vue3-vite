  ...
  code: [
    {
      required: true,
      trigger: 'blur',
      message: t('password.accountPlaceholder3')
    }
  ] // 验证码校验规则
})

// 是否开启验证码
const verifyEnable = ref(env.VITE_VERIFY_ENABLE === 'true')
const imgSrc = ref('')

//获取验证码图片
const getVerifyCode = () => {
  state.ruleForm.randomStr = generateUUID()
  imgSrc.value = `${env.VITE_API_URL}/code?randomStr=${state.ruleForm.randomStr}`
}
