      ...
      <el-row class="ml10" v-show="showSearch">
        <el-form :inline="true" :model="state.queryForm" ref="queryRef">
          ...
          <el-form-item :label="t('job.jobStatus')" prop="jobStatus">
            <el-select
              :placeholder="t('job.inputjobStatusTip')"
              v-model="state.queryForm.jobStatus">
              <el-option
                :key="index"
                :label="item.label"
                :value="item.value"
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template #default="scope">
            <dict-tag
              :options="job_status"
              :value="scope.row.jobStatus"></dict-tag>
          </template>
        </el-table-column>
        <el-table-column
          :label="t('job.jobExecuteStatus')"
          ...
          <template #default="scope">
            <dict-tag
              ...
              :value="scope.row.jobExecuteStatus"></dict-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column
          :label="t('job.previousTime')"
          prop="previousTime"
          show-overflow-tooltip
          width="120" />
        <el-table-column
          :label="t('job.nextTime')"
          prop="nextTime"
          show-overflow-tooltip
          width="120" />
        <el-table-column
        ...
        <el-table-column
          :label="t('job.executePath')"
          prop="executePath"
          show-overflow-tooltip
          width="120" />
        <el-table-column
          :label="t('job.className')"
          prop="className"
          show-overflow-tooltip
          width="120" />
        <el-table-column
        ...
        <el-table-column
          ...
          <template #default="scope">
            <dict-tag
              ...
              :value="scope.row.misfirePolicy"></dict-tag>
          </template>
        </el-table-column>

        <el-table-column :label="$t('common.action')" fixed="right" width="300">
          <template #default="scope">
            <el-button @click="handleJobLog(scope.row)" text type="primary">
              日志
            </el-button>
            ...
            <el-button
              v-auth="'job_sys_job_edit'"
              @click="handleEditJob(scope.row)"
              text
              type="primary">
              {{ $t('common.editBtn') }}
            </el-button>
...
import {
  ...
  shutDownJobRa,
  startJobRa
} from '@jmyg/api/daemon/job'
import { useMessage, useMessageBox } from '@hooks/message'
import { useDict } from '@hooks/dict'
import { useI18n } from 'vue-i18n'
defineOptions({ name: 'systemSysJob' })
// 引入组件
...
const handleDelete = async (row) => {
  ...
  try {
    await useMessageBox().confirm(
      `${t('common.delConfirmText')}(任务名称:${jobName})`
    )
  } catch {
    return
  }
