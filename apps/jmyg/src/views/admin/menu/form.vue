    ...
    <el-form
      ...
      <el-form-item :label="$t('sysmenu.parentId') + '：'" prop="parentId">
        <el-tree-select
          ...
          clearable
          check-strictly
          :placeholder="$t('sysmenu.inputParentIdTip')"></el-tree-select>
      </el-form-item>
      <el-form-item :label="$t('sysmenu.name') + '：'" prop="name">
        <el-input
          v-model="state.ruleForm.name"
          clearable
          :placeholder="$t('sysmenu.inputNameTip')"></el-input>
      </el-form-item>
      <el-form-item :label="$t('sysmenu.enName') + '：'" prop="enName">
        <el-input
          v-model="state.ruleForm.name"
          clearable
          :placeholder="$t('sysmenu.inputEnNameTip')"></el-input>
      </el-form-item>
      <el-form-item
        :label="$t('sysmenu.path') + '：'"
        prop="path"
        v-if="state.ruleForm.menuType === '0'">
        <el-input
          v-model="state.ruleForm.path"
          :placeholder="$t('sysmenu.inputPathTip')" />
      </el-form-item>
      <el-form-item
        :label="$t('sysmenu.permission') + '：'"
        prop="permission"
        v-if="state.ruleForm.menuType === '1'">
        <el-input
      ...
      <el-form-item
        ...
        v-if="state.ruleForm.menuType === '0'">
        <IconSelector
          :placeholder="$t('sysmenu.inputIconTip')"
      ...
      <el-form-item
        ...
        v-if="
          ...
          state.ruleForm.path?.startsWith('http')
        ">
...
import { info, pageList, putObj, addObj } from '@jmyg/api/admin/menu'
...
const onSubmit = async () => {
  ...
  try {
    state.ruleForm.menuId
      ? await putObj(state.ruleForm)
      : await addObj(state.ruleForm)
    useMessage().success(
      t(
        state.ruleForm.menuId
          ? 'common.editSuccessText'
          : 'common.addSuccessText'
      )
    )
    visible.value = false
    emit('refresh')
