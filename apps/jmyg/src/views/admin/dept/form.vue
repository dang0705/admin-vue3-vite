<template>
  <div class="system-dept-dialog-container">
    <el-dialog
      :title="dataForm.deptId ? $t('common.editBtn') : $t('common.addBtn')"
      v-model="visible"
      width="600">
      <el-form
        ...
        <el-form-item :label="$t('sysdept.name') + '：'" prop="name">
          <el-input
            ...
            :placeholder="$t('sysdept.inputnameTip')"
            clearable />
        </el-form-item>
...
import { getObj, deptTree, addObj, putObj } from '@jmyg/api/admin/dept'
...
const onSubmit = async () => {
  ...
  try {
    ...
    useMessage().success(
      t(dataForm.deptId ? 'common.editSuccessText' : 'common.addSuccessText')
    )
    visible.value = false
    emit('refresh')
  } catch (err: any) {
