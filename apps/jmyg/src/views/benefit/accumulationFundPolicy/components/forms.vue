...
const save = async () => {
  ...
  if (route.query.type === 'see' || route.query.type === 'addVersion') {
    BottomFormData.value.effectTime = titleFormData.value.effectTime,
    BottomFormData.value.remark = titleFormData.value.remark
  }
  route.query.id ? await putObj({
    ...titleFormData.value,
    id: route.query.id,
    insuredArea,
    itemList: data.value,
    version: BottomFormData.value
  })
