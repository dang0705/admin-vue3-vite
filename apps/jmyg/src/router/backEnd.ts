...
import {
  ...
  notFoundAndNoPower,
  dynamicRoutes
} from '@jmyg/router/route'
import {
  formatTwoStageRoutes,
  formatFlatteningRoutes,
  router
} from '@jmyg/router/index'
import { useRoutesList } from '@stores/routesList'
import { useTagsViewRoutes } from '@stores/tagsViewRoutes'
import { useMenuApi } from '@jmyg/api/admin/menu'
...
export async function initBackEndControlRoutes() {
  ...
  // 处理路由（component），替换 baseRoutes（@jmyg/router/route）第一个顶级 children 的路由
...
function setCacheTagsViewRoutes() {
  ...
  storesTagsView.setTagsViewRoutes(
    formatTwoStageRoutes(formatFlatteningRoutes(baseRoutes))[0].children
  )
}

/**
 * 处理路由格式及添加捕获所有路由或 404 Not found 路由
 * @description 替换 baseRoutes（@jmyg/router/route）第一个顶级 children 的路由
 * @returns 返回替换后的路由数组
 */
function setFilterRouteEnd() {
  let filterRouteEnd: any = formatTwoStageRoutes(
    formatFlatteningRoutes(baseRoutes)
  )
 ...
 * @description 此处循环为 baseRoutes（@jmyg/router/route）第一个顶级 children 的路由一维数组，非多级嵌套
  ...
  return routes.map((item: any) => {
    if (item.path && item.path.startsWith('http')) {
      if (item.meta.isIframe) {
        item.component = () => import('@layouts/routerView/iframes.vue')
      } else {
        item.component = () => import('@layouts/routerView/link.vue')
      }
