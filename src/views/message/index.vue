...
<template>
	<Table-view module="docs/message.ts" :columns="columns" no-border noFormView no-header :border="false" v-model="selectObjs">
		...
		<template #title-header="{ refresh }">
			<el-button text @click="delMessage(refresh)" :disabled="!selectObjs.length">删除</el-button>
		</template>
		<template #title="{ row }">
			<div class="rounded-[3px] relative" :style="bgc(row.noticeType)">
				{{ row.title }}
				<div v-if="row.readStatus === '0'" class="w-[6px] h-[6px] rounded-[50%] bg-[#FF6826] absolute top-[-3px] left-0"></div>
			</div>
		</template>
		...
		<template #right-header="{ refresh }">
			<el-button type="primary" @click="readMarkAll(refresh)">标记全部已读</el-button>
		</template>
		<template #right="{ row, refresh }">
			<div class="text-[#0065FF] cursor-pointer" @click="goDetail(row.id, row.url, refresh)">详情 &nbsp; &gt;</div>
