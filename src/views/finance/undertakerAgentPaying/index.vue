...
const confirmToPay = async (type = 'batch', { id, undertakerName, undertakerCard, undertakerBankName, undertakerBankNumber, paidAmount }: any) => {
	if (type === 'batch') {
		...
		dialog.value = {
			...
			show: true,
			forms: [
				{ label: '付款开户行', control: 'el-input', key: 'sendBankName', value: selectedRows.value[0].sendBankName },
				{ label: '付款银行账号', control: 'el-input', key: 'sendAccountNumber', value: selectedRows.value[0].paymentBankId },
				{
					label: '批量支付总金额（元）',
					control: 'el-input',
					key: 'paidAmount',
