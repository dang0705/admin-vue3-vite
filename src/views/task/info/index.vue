...
<template>
  <TableView isTab :columns="columns" module="core/task.ts" :condition-forms="conditionForms" :actions="computedActions" labelWidth="120px">
    ...
    <template #TaskType="{ formData }">
      <el-form-item label="任务类型">
        <TaskType v-model:taskTypeFirst="formData.taskTypeFirst" v-model:taskTypeSecond="formData.taskTypeSecond" />
      </el-form-item>
    </template>
    ...
    <uploadExcel ref="addUnderTakerRef" guidance="请先确保待指派的承接人已录入系统且已签署任务的承接服务商，然后按照导入模版填写承接人信息。" upload-label="待签署用户名单" upload-url="core/undertakerTask/batchAppointUndertaker"
      temp-url="/files/批量导入承接人模板.xlsx" template-on-front :params="params" title="批量指派承接人" />
  </TableView>
</template>
