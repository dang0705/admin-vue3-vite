...
const data = computed(() => (props.item?.data?.length ? props.item.data : localData.value));
	...
	<div class="section" v-loading="loading">
		...
		<ul class="flex min-h-[68px]">
			<li
				...
				:key="label"
				:style="{ color: item.statusMap[+attributeVal], width: `calc(100% / ${data.length})` }"
			>
				<span v-text="label" class="text-[14px] leading-10" />
				<span
					v-text="value"
					class="text-[28px] leading-[1] cursor-pointer hover:underline"
					@click="$router.push({ path: item.route, state: { tabLabel: label } })"
				/>
			</li>
