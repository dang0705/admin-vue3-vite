<template>
  <Table-view :condition-forms="conditionForms" :columns="columns" :actions="actions" label-width="140" module="core/spInfo">
    <template #top-bar>
      <el-button type="primary" class="ml10" @click="$router.push({ path: '/sp/info/add' })" v-auth="'core_spInfo_add'">
        +添加服务商
      </el-button>
    </template>
    <template #hasPaymentChannel="{ row: { hasPaymentChannel, statusDesc, id } }">
    ...
    <Dialog title="添加支付通道" v-model="visible" v-model:form-data="form" force-update :close-on-click-modal="false" :forms="forms" :on-submit="onSubmit" :columns="12" :labelWidth="140" vertical draggable
      width="60%" ref="dataFormRef" submit-button-text="确认" button-position="center" />
  </Table-view>
</template>
