<template>
  <div class="layout-padding">
        ...
        <el-form :inline="true" :model="state.queryForm" ref="queryRef">
          <el-form-item :label="$t('spInfo.spName')" prop="spName">
            <InputPlus
              ...
              v-model="state.queryForm.spName" />
          </el-form-item>
          <el-form-item :label="t('spInfo.status')" class="ml2" prop="status">
            <el-select v-model="state.queryForm.status">
              <el-option
                ...
                :label="item.label"
                :value="item.value"
                v-for="(item, index) in sp_status"></el-option>
            </el-select>
          </el-form-item>
          <el-form-item>
            <el-button
            ...
            <el-button v-debounce="resetQuery" formDialogRef icon="Refresh">
              {{ $t('common.resetBtn') }}
            </el-button>
          </el-form-item>
        </el-form>
      ...
      <el-table
        ...
        <el-table-column label="是否开启支付通道" width="300">
          <template #default="scope">
            ...
            <div
              v-else-if="
                scope.row.hasPaymentChannel === '0' &&
                scope.row.statusDesc === '停用'
              ">
              否
            </div>
            <el-button v-else text type="primary" @click="openDialog">
              否，立即前往开通
            </el-button>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="300" fixed="right">
          <template #default="scope">
            ...
            <el-button
              ...
              @click="
                deactivateShow(scope.row.id, scope.row.spName, scope.row.status)
              ">
    ...
    <el-dialog v-model="deactivateVisible" title="停用服务商" width="40%">
      <div class="mb-5">
        您确定要停用服务商"{{ deactivateInfo.spName }}"吗？
      </div>
      <div>
        停用后服务商不可再承接新的任务；不可签署新的承接人；不可与商户添加新的服务协议
      </div>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="deactivateVisible = false">取消</el-button>
          <el-button type="primary" v-debounce="handleDeactivate">
            确定
          </el-button>
        </span>
      </template>
...
const handleDeactivate = async () => {
  try {
    ...
    getDataList()
    useMessage().success(
      deactivateInfo.value.status === '1' ? '停用成功' : '启用成功'
    )
