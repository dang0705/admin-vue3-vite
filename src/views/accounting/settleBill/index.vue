<template>
  <TableView :condition-forms="conditionForms" :actions="actions" :columns="columns" action-body="账单" module="core/settleBill.ts" isTab labelWidth="120px">
    <template #top-bar="{ otherInfo }">
      <el-button v-auth="'core_settleBill_add'" style="margin-right: 24px" type="primary" class="ml10" @click="openDialog()">
        导入结算
      </el-button>
      <div class="info_list">
        <div class="info_item" v-for="(item, index) in otherInfo.sumResp" :key="index">
          {{ item.label }}:{{ item.value }}元
        </div>
      </div>
      <el-button v-auth="'core_settleBill_batch_export'" style="margin-right: 24px" type="primary" class="ml10" @click="handleBtn">
        批量导出账单
      </el-button>
      <el-button v-auth="'core_settleBill_batch_export_detail'" style="margin-right: 24px" type="primary" class="ml10" @click="handleBtn">
        批量导出明细
      </el-button>
    </template>
    <!-- 导入结算-->
    <uploadExcel ref="importBillRef" guidance="在批量结算之前，请确认所有任务承接已完成交付，然后请下载《任务承接明细表模版》，按照参考格式填写并在本页面上传" upload-label="导入结算" upload-url="core/settleBill/import" temp-url="/files/任务承接明细表.xlsx"
      template-on-front title="导入结算" label-width="178px" :forms="excelForms">
      <template #merchantId="{ formData }">
        <el-form-item label="结算商户" prop="merchantId" :rules="[
      ...
      <template #paymentBankId="{ formData }">
        <el-form-item prop="paymentBankId" label="支付通道" :rules="[
      ...
      <template #platformBankId="{ formData }">
        <el-form-item prop="platformBankId" label="平台支付通道" :rules="[
