...
const transfer = (arr: []) => {
  const result: any = {}
  arr.forEach(
    (item: Options<string, number>) => (result[item.value] = item.label)
  )
  return result
}
/**
  ...
  if (dic) {
    ...
    return computed(() => {
      for (const dictsKey in dicts) {
        const map = {} as any
        dicts[dictsKey].value.forEach(
          ({ value, label }: Options<string, number>) => (map[value] = label)
        )
        result[dictsKey] = map
      }
      return result
...
export const valueAsLabel = (object: Record<string, any>) => {
  ...
  for (const key in object) {
    revertedObj[object[key]] = /^\d+$/.test(key)
      ? parseInt(key)
      : (revertedObj[object[key]] = key)
  }
  return revertedObj
}
