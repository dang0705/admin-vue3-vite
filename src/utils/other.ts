 ...
 * @description 预览文件
 * @param url   路径
 * @param mime  文件类型(默认值: application/pdf)
 */

export async function previewFile({ url, mime = 'application/pdf' }: Record<string, string>) {
	let blob = new Blob([await $http.get(url, { responseType: 'blob' })], { type: `${mime};charset=utf-8` });
